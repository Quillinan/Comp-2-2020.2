...
//
    ...
    //LAB 8
    @Test
    public void testeExceptionsTuitando(){
        StringBuilder sb = new StringBuilder();
        for (int i = 1; i <= TuiterLite.TAMANHO_MAXIMO_TUITES + 1; i++) {
            sb.append("t");
        }
        String texto = sb.toString();
        try{
            ...
            assertEquals(TuiterLite.TAMANHO_MAXIMO_TUITES + 1,e.getTamanhoTexto());
        } catch (UsuarioDesconhecidoException e){
            fail("Usuario registrado");
        }
        try{
            Usuario usuarioNaoRegistrado = new Usuario("Não Registrado","fake@email.com");
            tuiterLite.tuitarAlgo(usuarioNaoRegistrado,"Teste");
            fail("Usuario não registrado");
        } catch (UsuarioDesconhecidoException e){
            //Passou
        } catch (TamanhoMaximoExcedidoException e){
            fail("String excedendo tamanho permitido");
        }
