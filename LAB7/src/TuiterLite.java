...
public class TuiterLite {
    ...
    private static Set<Usuario> usuariosCadastrados;
    private static Set<String> emailsCadastrados;
    private static Set<Tuite> tuitesPlataforma;
    private static Map<String,Integer> vezesByHashtags;

    public TuiterLite() {
        usuariosCadastrados= new HashSet<>();
        emailsCadastrados = new HashSet<>();
        tuitesPlataforma = new HashSet<>();
        vezesByHashtags = new HashMap<>();
    }
    /**
    ...
    public Tuite tuitarAlgo(Usuario usuario, String texto) {
        if(!usuariosCadastrados.contains(usuario) || texto.length() > TAMANHO_MAXIMO_TUITES){
            return null;
        }
        else{
            ...
            Tuite tuiteNovo = new Tuite(usuario,texto,hashtags);
            tuitesPlataforma.add(tuiteNovo);
            for(String hashtag: hashtags){
                if(vezesByHashtags.containsKey(hashtag)){
                    vezesByHashtags.put(hashtag,vezesByHashtags.get(hashtag)+1);
                }
                else{
                    vezesByHashtags.put(hashtag,1);
                }
            }
            return tuiteNovo;
        }
    ...
    public String getHashtagMaisComum() {
        if(vezesByHashtags.size()==0){
            return null;
        }
        int value = 0;
        String hashtagPopular = null;
        Set<String> hashtags = vezesByHashtags.keySet();
        for(String hashtag: hashtags){
            if(vezesByHashtags.get(hashtag)>value){
                value = vezesByHashtags.get(hashtag);
                hashtagPopular = hashtag;
            }
        }
